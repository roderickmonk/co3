...
# The following allows to this pytest to run on a server
...
def pytest_successful(self, episode: int, eval_interval=10, eval_episodes=10) -> bool:
...
def test_ddpg_with_copter():

    logger.setup()

    config = DdpgPytestConfig(
        **{
            "instance_id": 0,
            "agent": {
                "torch_models": {
